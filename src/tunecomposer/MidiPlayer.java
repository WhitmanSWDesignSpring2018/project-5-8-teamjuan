...
public class MidiPlayer {
    ...
    /**
     * adds a new MidiEvent to the current composition. Assumes all parameters are
     * legal values (in the appropriate range). This method is very low level but
     * gives you greater freedom to add almost any kind of Midi event you wish.
     *
     ...
     */
    public void addMidiEvent(int status, int data1, int data2, int startTick, int trackIndex) {
     ...
     *                   to a different instrument. The default instrument for all
     *                   channels is the grand piano.
     * @param trackIndex an integer from 0 to 7 giving the track for the note
     */
    public void addNote(int pitch, int volume, int startTick, int duration, int channel, int trackIndex) {
        addMidiEvent(ShortMessage.NOTE_ON + channel, pitch, volume, startTick, trackIndex);
        addMidiEvent(ShortMessage.NOTE_OFF + channel, pitch, volume, startTick + duration, trackIndex);
    }

    /**
     * plays all the Midi events in all the tracks of this composition immediately
     */
    public void play() {
    ...
    public void clear() {
        try {
            sequencer.setSequence(new Sequence(Sequence.PPQ, getResolution(), NUM_TRACKS));
        } catch (InvalidMidiDataException e) {
            e.printStackTrace();
        }
    }
